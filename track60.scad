...
// Added: O - mirrored B
// Added: p - one half of a double-track curve
// Added: q - one half of a double-track straight
...
// Added: + - rotate remainder of the string by 60 degrees
// Added: = - rotate remainder of the string by -60 degrees
...
module decode_shortname(s, i, radius, surface, part, flip_mask=0, is_even=false) {
  off = len(s) - 1;
  n = (len(s) <= 0 || s[off] == "-") ? 0 :
  ...
  } else if ((c == "P" && is_double && i == 0)) {
    dbl_curve60(radius, dir="left", surface=surface, part=part,
                   only_left=is_even, only_right=!is_even);
  } else if ((c == "Q" && is_double && i == 0)) {
    dbl_straight60(radius, surface, part,
                   only_left=is_even, only_right=!is_even);
  }
}
...
module dbl_straight60(radius, surface="road-rail", part="all",
  ...
  track_parts(radius, surface, part, mirror_symmetric=true) {
    ...
    if (startswith(part, "split-")) {
      ...
      rots = only_right ? [0] : only_left ? [180] : [0,180];
      for (i=rots) rotate([0,0,i]) {
...
module dbl_curve60(radius, dir, surface="road-rail", part="all",
                   trim_ties=true, only_left=false, only_right=false) {
  epsilon=.1;

  if (dir=="right") {
    scale([-1,1,1])
      dbl_curve60(radius, "left", surface, part, trim_ties=trim_ties,
    ...
    translate([-radius,-straight_length(radius)/2,0]) {
      rots = only_right ? [1] : only_left ? [-1] : [1,-1];
      for (i=rots) {
        ...
        rotate([0,0,(i+1)*30])
          translate([radius,0,0]) rotate([0,0,(i+1)*90])
            dbl_connector(surface=surface, part=part);
      }
    }
  }
